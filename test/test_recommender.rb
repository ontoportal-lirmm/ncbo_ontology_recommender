...
class TestRecommender < TestCase
  ...
  def test_recommend_validation
    ...
    assert_raises(ArgumentError) {@@recommender.recommend(nil, input_type, output_type, max_elements_set,
                                              ontologies, wc, ws, wa, wd)}
    assert_raises(ArgumentError) {@@recommender.recommend([ ], input_type, output_type, max_elements_set,
                                                          ontologies, wc, ws, wa, wd)}
    assert_raises(ArgumentError) {@@recommender.recommend(input, 0, output_type, max_elements_set,
                                                          ontologies, wc, ws, wa, wd)}
    assert_raises(ArgumentError) {@@recommender.recommend(input, input_type, 0, max_elements_set,
                                                          ontologies, wc, ws, wa, wd)}
    assert_raises(ArgumentError) {@@recommender.recommend(input, input_type, 2, 0,
                                                       ontologies, wc, ws, wa, wd)}
    assert_raises(ArgumentError) {@@recommender.recommend(input, input_type, 2, nil,
                                                          ontologies, wc, ws, wa, wd)}
    assert_raises(ArgumentError) {@@recommender.recommend(input, input_type, output_type, max_elements_set,
                                                          ontologies, -1, ws, wa, wd)}
    assert_raises(ArgumentError) {@@recommender.recommend(input, input_type, output_type, max_elements_set,
                                                          ontologies, wc, -1, wa, wd)}
    assert_raises(ArgumentError) {@@recommender.recommend(input, input_type, output_type, max_elements_set,
                                                          ontologies, wc, ws, -1, wd)}
    assert_raises(ArgumentError) {@@recommender.recommend(input, input_type, output_type, max_elements_set,
                                                          ontologies, wc, ws, wa, -1)}
    assert_raises(ArgumentError) {@@recommender.recommend(input, input_type, output_type, max_elements_set,
                                                          ontologies, 0, 0, 0, 0)}
  ...
  def test_recommend_query_text_single
    ...
    ontologies = []
    wc, ws, wa, wd = [0.55, 0.15, 0.15, 0.15]
    recommendations = @@recommender.recommend(input, input_type, output_type, max_elements_set, 
                                              ontologies, wc, ws, wa, wd)
    # Expected ranking:
